...
#* Get All Orders for Admin [OK]
@orders_bp.route('/admin/all', methods=['GET'])  # GET /api/orders/admin/all
@admin_required
def get_all_orders():
    try:
...
#* Get Order Items for a Specific Order [OK]
@orders_bp.route('/admin/items/<int:order_id>', methods=['GET'])  # GET /api/orders/admin/items/<order_id>
@admin_required
def get_order_items_by_order(order_id):
    try:
        ...
        if not order_items:
            return jsonify({"message": "No order items found for this order."}), 404
...
@orders_bp.route('/admin/shipped/<int:order_id>', methods=['PUT'])  # PUT /api/orders/admin/shipped/<order_id>
@admin_required
def change_to_shipped(order_id):
    user_id = get_jwt_identity() 
    try:
        print(f"User ID: {user_id}, Order ID: {order_id}")

        # Check order for order id
        ...
        # Check if the current status is "Order Confirmed"
        if order['status'] != 'Order Confirmed':
            print(f'The order status is currently: {order["status"]}')
            print("Order status is not 'Order Confirmed'.")
            return jsonify({"error": "Order must be 'Order Confirmed' to be marked as 'Shipped'."}), 400
